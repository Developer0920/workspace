...
1)EMPLOYEES 테이블에서 입사한 달(hire_date) 별로 인원수를 조회하시오 . 
  ...
  SELECT to_char(hire_date, 'mm') as "입사한 달", count(employee_id) as "인원수"
  FROM employees  
...
SELECT nvl(d.department_name, '<미배치인원>') as "부서명", count(employee_id) as "직원수"
...
단, 관리자가 없는 직원이 있다면 그 직원 정보도 출력결과에 포함시켜야 합니다.
 ...
 SELECT e.first_name as "직원명", m.first_name as "대장명"
