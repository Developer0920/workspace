...
public class Prob02_class {
	public static void main(String[] args) {
		SalesGrade[] sg = new SalesGrade[4]; //클래스 배열 생성
		sg[0] = new SalesGrade(1000, 2000, 'A'); //인스턴스 생성
		sg[1] = new SalesGrade(2001, 3000, 'B'); //인스턴스 생성
		sg[2] = new SalesGrade(3001, 4000, 'C'); //인스턴스 생성
		sg[3] = new SalesGrade(4001, 5000, 'D'); //인스턴스 생성
	...
	public static void display(SalesGrade[] sg, String name, int sales) {
		...
		for(SalesGrade data : sg) {
			if(sales>=data.losal && sales<=data.hisal) {
				System.out.printf("%s님의 등급은 %c등급입니다.\n", name, data.grade);
			}			
		}
	}// end display()
