package java0521_collection.prob;


import java.io.File;
import java.io.FileNotFoundException;
import java.util.ArrayList;
import java.util.Scanner;

/*
 * [문제]
 * phone.txt파일에 저장된 값들을  phoneProduct()메서드에서
 * SmartPhone에 저장한 후 ArrayList에 추가한후 리턴하고,
 * prnDisplay()메소드에서 프로그램 실행결과처럼 출력하는 프로그램을
 * 구현하시오.
 * 
 * [프로그램 실행결과]
 * << 1 번째 상품 >>
	제품 아이디 : PROD-00001
	제품명 : 아이폰5
	가격 : 940000
	수량 : 4
	제조사 : 애플
   << 2 번째 상품 >>
	제품 아이디 : PROD-00002
	제품명 : 갤럭시S
	가격 : 860000
	수량 : 3
	제조사 : 삼성전자

 */
public class Prob003_ArrayList {

	public static void main(String[] args) {
		String pathFile=".\\src\\java0521_collection\\prob\\phone.txt";
		ArrayList<SmartPhone> phoneList = phoneProduct(pathFile);
		prnDisplay(phoneList);
	}//end main()
	
	private static ArrayList<SmartPhone> phoneProduct(String pathFile) {
		// phone.txt파일의 데이터를 ArrayList에 저장한후 리턴하는 프로그램을 구현하시오.
		ArrayList<SmartPhone> as = new ArrayList<SmartPhone>();
		Scanner scan;
		try {
			scan = new Scanner(new File(pathFile));
			while(scan.hasNext()) {
				String stn = scan.nextLine();
				String[] str = stn.split(":");
				SmartPhone sp = new SmartPhone();
				sp.setProductId(str[0]);
				sp.setName(str[1]);
				sp.setPrice(Integer.parseInt(str[2]));
				sp.setAmount(Integer.parseInt(str[3]));
				sp.setMaker(str[4]);
				as.add(sp);
				
			}
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return as;
	}//end phoneProduct( )
	
	private static void prnDisplay(ArrayList<SmartPhone> phoneList){
		//phoneList매개변수의 저장된 값을 출력하는 프로그램을 구현하시오.	
		for(int i=0; i<phoneList.size(); i++) {
			System.out.printf("<< %d번째 상품 >>\n",i+1);
			System.out.print("제품아이디:"+ phoneList.get(i).getProductId()+"\n");
			System.out.print("제품명:"+ phoneList.get(i).getName()+"\n");
			System.out.print("가격:"+ phoneList.get(i).getPrice()+"\n");
			System.out.print("수량:"+ phoneList.get(i).getAmount()+"\n");
			System.out.print("제조사:"+ phoneList.get(i).getMaker()+"\n");
		}
	}//end prnDisplay( )

}//end class











